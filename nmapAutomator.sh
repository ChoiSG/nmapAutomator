...
$nmapType -T4 --max-retries 1 --max-scan-delay 20 --defeat-rst-ratelimit --open -oA nmap/Quick_$1.nmap $1
assignPorts $1
...
if [ -z `echo "${basicPorts}"` ]; then
	...
	$nmapType -sCV -p`echo "${basicPorts}"` -oA nmap/Basic_$1.nmap $1 
fi
...
$nmapType -sU --max-retries 1 --open -oA nmap/UDP_$1.nmap $1
assignPorts $1
...
if [ ! -z `echo "${udpPorts}"` ]; then
	...
	if [ -f /usr/share/nmap/scripts/vulners.nse ]; then
        	$nmapType -sCVU --script vulners --script-args mincvss=7.0 -p`echo "${udpPorts}"` -oA nmap/UDP_$1.nmap $1
	else
        	$nmapType -sCVU -p`echo "${udpPorts}"` -oA nmap/UDP_$1.nmap $1
	fi
fi
...
$nmapType -p- --max-retries 1 --max-rate 500 --max-scan-delay 20 -T4 -v -oA nmap/Full_$1.nmap $1
assignPorts $1
...
if [ -z `echo "${basicPorts}"` ]; then
	echo ""
        ...
        echo -e "${NC}"
        $nmapType -sCV -p`echo "${allPorts}"` -oA nmap/Full_$1.nmap $1
	assignPorts $1
else
	cmpPorts $1
	if [ -z `echo "${extraPorts}"` ]; then
		...
		echo ""
        	...
        	echo -e "${NC}"
        	$nmapType -sCV -p`echo "${extraPorts}"` -oA nmap/Full_$1.nmap $1
		assignPorts $1
	fi
fi
...
if [ ! -f /usr/share/nmap/scripts/vulners.nse ]; then
	...
	echo -e "${NC}"
	$nmapType -sV --script vulners --script-args mincvss=7.0 -p`echo "${ports}"` -oA nmap/CVEs_$1.nmap $1
	echo ""
fi
...
echo -e "${NC}"
$nmapType -sV --script vuln -p`echo "${ports}"` -oA nmap/Vulns_$1.nmap $1
echo -e ""
echo -e ""
echo -e ""
}
